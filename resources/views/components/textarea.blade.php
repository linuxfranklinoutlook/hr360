...
{{-- <div class="relative border border-gray-300 rounded-md px-3 py-3 shadow-sm focus-within:ring-1 focus-within:ring-blue-600 focus-within:border-blue-600">
	...
	<textarea {{ $disabled ? 'disabled' : '' }} {!! $attributes->merge(['class' => 'block w-full border-0 p-0 text-gray-900 placeholder-gray-400 focus:ring-0 sm:text-sm', 'placeholder' => 'Please enter ' . $label]) !!}>{{ $slot }}</textarea>
</div> --}}
<div>
