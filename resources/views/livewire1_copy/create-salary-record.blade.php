...
<div class="p-6">
	...
	<form action="{{ route('payslip.store') }}" method="POST">
		...
		<div class="my-3">
			<x-select label="Select Year" required name="year">
				@foreach ($years as $year)
					<option value="{{ $year }}"
						{{ $year === Carbon::now()->format('Y') ? 'selected' : '' }}>{{ $year }}</option>
				@endforeach
			</x-select>
		</div>
		<div class="my-3">
			<x-select label="Select Month" required name="month">
				@foreach ($months as $month)
					<option value="{{ $month }}"
						{{ $month === Carbon::now()->format('m') ? 'selected' : '' }}>{{ $month }}</option>
				@endforeach
			</x-select>
		</div>
		<div class="my-3">
			<x-select label="Employee" required name="employee_id">
				@foreach (\App\Models\Employee::all() as $emp)
					<option value="{{ $emp->id }}">
						{{ $emp->getFullName() . '(' . $emp->personal_email . ')' }}</option>
				@endforeach
			</x-select>
		</div>
		<div class="my-3">
		...
		<div class="my-3">
			<h3 class="text-sm font-semibold">Net-in-hand will be calculated in the next step, after all the
				deductions.</h3>
		</div>
