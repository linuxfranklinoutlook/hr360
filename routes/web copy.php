...
Route::get('/dashboard_admin', [DashboardController::class,"index"])->middleware(['auth'])->name('dashboard_test');
Route::prefix('learning-management')->middleware(['auth'])->group(function () {
...
Route::prefix('playground')->middleware(['auth'])->group(function () {
	...
	Route::get('pdf-test', function () {
		$pdf = PDF::loadView('payroll._pdfview');
		return $pdf->stream();
		// return view('payroll._pdfview');
	});
});

Route::prefix('documents-and-media/{media}')->middleware(['auth'])->group(function () {
	Route::get('delete-media', [DocumentAndMediaController::class, "deleteFile"])->name('delete-file');
});

// Route::get('admin/employee/show_id', [EmployeeController::class, 'show_id']);
// Route::get('generate-pdf', [EmployeeController::class, 'generatePDF']);
Route::get('/generate_pdf', [EmployeeController::class, 'createPDF']);
Route::put('salary/calculate', [SalaryStructureController::class, 'calculate'])->middleware(['auth']);
// Route::get('admin/tasks/{id}/done', [TasksController::class,'done'])->middleware(['auth']);
